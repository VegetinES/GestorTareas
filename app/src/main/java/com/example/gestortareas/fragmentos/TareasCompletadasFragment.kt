...
// Fragmento que muestra la lista de tareas completadas
class TareasCompletadasFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_tareas_completadas, container, false)

        // Inicializa las referencias a las vistas
        recyclerView = view.findViewById(R.id.recycler_tareas_completadas)
        tvSinTareas = view.findViewById(R.id.tv_sin_tareas_completadas)
    ...
    private fun cargarTareasCompletadas() {
        dbHelper = TareasDBHelper(requireContext())
        val tareas = dbHelper.obtenerTareasCompletadas()

        // Muestra un mensaje si no hay tareas completadas
        if (tareas.isEmpty()) {
